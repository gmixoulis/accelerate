...
const DynamicGrid = ({ params: { tableName } }) => {
  ...
  const fetchData = async () => {
    let response = await axiosRequest(`${BACKEND_URL}${tableName}/`)
    let rawData
    if (response?.data[Pluralize(tableName)]) {
      response = response.data[Pluralize(tableName)]
      rawData = response
      console.log(rawData)
    } else {
      rawData =
